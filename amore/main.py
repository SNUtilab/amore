...
if __name__ == '__main__':
    ...
    for idx, row in data_.iterrows() :
        ...
        abstract = row['요약'][0:100]
        for idx_, row_ in data_.iterrows() :
            if idx != idx_ :
                abstract_ = row_['요약'][0:100]
    ...
    for time in [0,1,2] :
        data_sample = data_.loc[data_['application_span'] == time, :]
    ...
    data_sample = data_.loc[data_['application_span'] == 2, :]
    ...
    tunning_result = LDA_tunning.tunning(texts, keyword_dct, corpus, 10, 41, 3)
    tunning_result.to_csv('./output/lda_tune_result.csv',index = 0)
    
    
    #%% 6. LDA handling
    
    LDA_obj = LDA_tunning.LDA_obj(texts, 40, 1, 0.01) # best score 입력
    # import pickle
    ...
    pd.DataFrame(topic_time_topn).to_excel(writer , sheet_name = 'topic_time_topn', index = 1)
